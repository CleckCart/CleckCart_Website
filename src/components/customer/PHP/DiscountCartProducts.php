<?php
    ...
    if(isset($_GET['user']) && isset($_GET['id']) && isset($_GET['image']) && isset($_GET['name']) && isset($_GET['description']) && isset($_GET['price']) && isset($_GET['newPrice']) && isset($_GET['quantity'])){
        ...
        if($productStock <= 1){
            header("Location:./DiscountProductDetail.php?user=$productUser&id=$productId&name=$productName&description=$productDescription&image=$productImage&price=$productPrice&newPrice=$discountedPrice&stock=$productStock&quantity=$productQuantity&error=Item Out Of Stock");
        }
        else{
            //Getting User ID
            ...
            header("Location:./DiscountProductDetail.php?user=$productUser&id=$productId&name=$productName&description=$productDescription&image=$productImage&price=$productPrice&newPrice=$discountedPrice&stock=$productStock&quantity=$productQuantity&success=Added to Cart");
        }

    }
    else{
        header("Location:./DiscountProductDetail.php?user=$productUser&id=$productId&name=$productName&description=$productDescription&image=$productImage&price=$productPrice&newPrice=$discountedPrice&stock=$productStock&quantity=$productQuantity&error=Something went wrong");
    }
    ?>