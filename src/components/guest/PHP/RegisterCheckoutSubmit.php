                                                    ...
                                                    if(strlen($customerPhone)>=10 && strlen($customerPhone) < 12) 
                                                        {                                                           
                                                            ...
                                                            if(strcmp($customerPassword,$customerConfirmPassword)==0)
                                                                ...
                                                                {
                                                                    header("Location:./Register.php?cartId=$guestCartId&totalCartItems=$productTotalQuantity&collectionDate=$collectionDate&collectionTime=$collectionTime&error=Please make sure password are matched.");
                                                                }
                                                        ...
                                                        {
                                                            header("Location:./Register.php?cartId=$guestCartId&totalCartItems=$productTotalQuantity&collectionDate=$collectionDate&collectionTime=$collectionTime&error=Please enter a valid phone number.");
                                                        }
                                                }

                                            else
                                                {
                                                    header("Location:./Register.php?cartId=$guestCartId&totalCartItems=$productTotalQuantity&collectionDate=$collectionDate&collectionTime=$collectionTime&error=Please pick the date for date of birth.");
                                                }
                                            
                                        }
                                    else
                                        {
                                            header("Location:./Register.php?cartId=$guestCartId&totalCartItems=$productTotalQuantity&collectionDate=$collectionDate&collectionTime=$collectionTime&error=Please use alphabets only in lastname.");
                                        }        
                                }   
                                
                            else
                                {
                                    header("Location:./Register.php?cartId=$guestCartId&totalCartItems=$productTotalQuantity&collectionDate=$collectionDate&collectionTime=$collectionTime&error=Please use alphabets only in firstname.");
                                }
                            
                        }
                    else
                        {   
                            header("Location:./Register.php?cartId=$guestCartId&totalCartItems=$productTotalQuantity&collectionDate=$collectionDate&collectionTime=$collectionTime&error=Please make sure username is 5 - 30 characters.");                   
                        }
                }
            }
    ?>