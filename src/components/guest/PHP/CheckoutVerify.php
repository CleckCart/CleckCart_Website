<?php
    ...
    if(isset($_POST['CollectionDateSubmit'])){
        ...
        if(isset($_POST['time']) && isset($_POST['day'])){
            ...
            // Check if the collection date is on Wednesday, Thursday, or Friday
            $collectionDayOfWeek = date('l', strtotime($collectionDate));
            if ($collectionDayOfWeek === 'Wednesday' || $collectionDayOfWeek === 'Thursday' || $collectionDayOfWeek === 'Friday') {
                echo "Selected Date: " . $collectionDate;
                ...
                header("Location:./Checkout.php?error=Please select a date that falls on Wednesday, Thursday, or Friday.");
            }
